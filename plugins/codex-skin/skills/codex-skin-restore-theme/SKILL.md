...
4. After explicit confirmation, run only the matching platform wrapper with `theme launch --json`. The same external Runtime Supervisor path owns the restart and restore; do not run a second launch. After Codex reopens, run `status --json`. Report success only when `restartKind` is `restore`, `restartStatus` is `completed`, and `runtimeStatus` is absent or terminal rather than active.
