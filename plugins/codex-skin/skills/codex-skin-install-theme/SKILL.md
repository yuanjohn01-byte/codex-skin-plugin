...
7. After Codex reopens, run the platform wrapper with `status --json`. If `restartStatus` is `restart_approved` or `running`, wait briefly and re-check for no longer than 60 seconds; do not start another continuation. Treat apply as successful only when `restartStatus` is `completed`, `appliedThemePublicId`/`appliedThemeVersion` match the request, and `sessionStatus` is `active` for that same theme. If restart status is `failed`, report only its stable `restartErrorCode`.
8. When no restart is required, treat success only as one JSON result with `ok: true`, the requested `themePublicId`, a `themeVersion`, an `operationId`, and `sessionStatus: "active"`.
...
10. On `CS-FLOW-AUTH-001`, ask the user to finish or retry authorization. On `CS-FLOW-ACCESS-001`, explain that purchase did not become ready within the bounded wait. On any theme/apply failure, report the stable code without exposing local paths or credentials.
11. If apply fails after Codex mutation may have started, offer the dedicated offline Restore Skill.
