...
8. A previous completed or failed on-demand operation never requires Restore before a user requests another theme. A direct switch is another `theme apply NEW_ID` operation; it may need one restart, but it must not insert a Restore or a second restart.
