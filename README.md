...
The Gate B engine accepts only versioned manifest fields and declared local PNG/JPEG/WebP assets; theme packages cannot provide CSS, JavaScript, shell commands, selectors, or remote execution URLs. The engine transaction follows `validate → stage → backup → apply → verify → commit`; restart consent is terminal before mutation. After confirmation, the detached Helper stops only the exact verified official Codex instance, rediscovers a stable signed installation, launches the same user profile on loopback CDP, waits for a real renderer, writes the fixed current-document style, verifies the exact theme, commits, and exits. Current templates carry their own scoped colour scheme: a theme operation never rewrites the user's native `appearanceTheme`, and a verified controlled window can switch directly across light/dark themes without Restore or a mode restart. A reload or untrusted ordinary window takes at most one confirmed restart to regain the loopback-controlled renderer. A new request may replace only an unconfirmed restart request; an approved or running one remains non-preemptible. Status records only the last visibly verified operation plus a current restart request; neither downloaded nor desired state alone is success. A bounded redacted terminal-restart history preserves the stable failure code if a later Restore replaces `current.json`.
