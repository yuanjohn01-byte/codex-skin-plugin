...
The Founder Staging repair closure uses the immutable Helper revision `0.1.0-paid-alpha.2`, Bootstrap revision `0.1.0-paid-alpha.1`, and its Staging API origin. The earlier signed `.1` Helper candidate remains immutable and is not overwritten. Neither Staging revision may be republished with Production bytes under the same tag or filename. Production promotion requires higher immutable Helper and Bootstrap revisions, the approved Production origin, and the release gate for those exact artifacts; the Plugin version remains `0.1.0-paid-alpha`.
...
The Gate B engine accepts only versioned manifest fields and declared local PNG/JPEG/WebP assets; theme packages cannot provide CSS, JavaScript, shell commands, selectors, or remote execution URLs. A verified package follows `validate → stage → backup → apply → verify → commit`. First use and cached reuse both require the exact embedded keyset, descriptor signature, package hash, manifest hash and compatible minimum engine version; callers cannot substitute a self-signed keyset. Template v5 synchronizes the theme's declared light/dark mode with Codex's native `appearanceTheme`, preserves the exact pre-theme appearance settings for Restore, bridges Codex's native dropdown/popover token family, and installs one fixed in-renderer controller before navigation. That controller uses a constructable stylesheet with an owned-style fallback, repairs route/DOM/style remounts, pauses on Appearance settings, resumes on return, and fully cleans up on switch or Restore; it is not a daemon and does not accept remote code. Verification measures computed contrast only for fixtures that are actually present, reporting absent late-rendered activity and diff regions as `not_present` instead of a synthetic pass. If apply, verify or commit fails, rollback uses a bounded context detached from request cancellation, verifies the restored snapshot or official state, restores the original appearance settings, and restores the previous desired-theme pointer. An incomplete rollback keeps its mutation-stage journal recoverable. The independent `theme restore` command removes the controller, fixed theme marker and background without network, login, access entitlement, Node, or Plugin cache access. Formal user exposure remains deferred to the later flow/release gates.
