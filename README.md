...
The Gate B engine accepts only versioned manifest fields and declared local PNG/JPEG/WebP assets; theme packages cannot provide CSS, JavaScript, shell commands, selectors, or remote execution URLs. The engine transaction follows `validate → stage → backup → apply → verify → restore-native-appearance → reverify → commit`; restart consent is terminal before mutation. After confirmation, the detached Helper stops only the exact verified official Codex instance, rediscovers a stable signed installation, launches the same user profile on loopback CDP, waits for a real renderer, writes the fixed current-document style, verifies the exact theme, restores the original on-disk appearance bytes, re-verifies, commits, and exits. A direct theme switch is another replacement transaction: it does not insert Restore because a historical runtime/session failure exists. Status records only the last visibly verified operation plus a current restart request; neither downloaded nor desired state alone is success.
