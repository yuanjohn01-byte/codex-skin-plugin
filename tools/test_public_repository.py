...
def write_baseline(fixture: Path) -> None:
    ...
    (scripts / "bootstrap-pins.ps1").write_text(
        "# Generated by tools/render_bootstrap_pins.py; do not edit by hand.\n"
    ...
    contracts = {
        ...
        "helper-release-verification-keyset-v1.schema.json": {
            "$schema": "https://json-schema.org/draft/2020-12/schema",
            "required": ["schemaVersion", "keys"],
            "additionalProperties": False,
        },
        "helper-release-verification-keys-v1.json": {
            "schemaVersion": 1,
            "keys": [
                {
                    "keyId": "helper-fixture-2026-08",
                    "algorithm": "Ed25519",
                    "usage": "helper-release",
                    "publicKeyBase64": "AAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAAA=",
                    "notBefore": "2026-08-01T00:00:00Z",
                    "notAfter": "2028-08-01T00:00:00Z",
                    "status": "active",
                }
            ],
        },
    ...
    helper_trusted_key_bytes = (
        contract_root / "helper-release-verification-keys-v1.json"
    ).read_bytes()
    helper_trusted_embed = fixture / "internal/release/trusted-verification-keys-v1.json"
    helper_trusted_embed.parent.mkdir(parents=True)
    helper_trusted_embed.write_bytes(helper_trusted_key_bytes)
    manifest_artifacts.append(
        {
            "destination": "internal/release/trusted-verification-keys-v1.json",
            "sha256": hashlib.sha256(helper_trusted_key_bytes).hexdigest(),
            "source": "codex-skin/contracts/public/helper-release-verification-keys-v1.json",
        }
    )
    fixtures = {
...
def negative_mixed_bootstrap_profile() -> None:
    with tempfile.TemporaryDirectory(prefix="codex-skin-public-bootstrap-profile-") as directory:
        fixture = Path(directory)
        initialized = run(["git", "init", "--quiet"], fixture)
        if initialized.returncode != 0:
            raise AssertionError(initialized.stderr)
        write_baseline(fixture)
        shell = fixture / "plugins/codex-skin/scripts/bootstrap-pins.sh"
        shell.write_text(
            shell.read_text(encoding="utf-8").replace(
                "codex-skin-staging.yuanjohn01.workers.dev",
                "codexskin.ai",
            ),
            encoding="utf-8",
        )
        ...
        checked = run([sys.executable, str(VALIDATOR), "--root", str(fixture)], fixture)
        combined = checked.stdout + checked.stderr
        expected = "Bootstrap pins do not match one fixed Staging or Production release profile"
        if checked.returncode == 0 or expected not in combined:
            raise AssertionError(f"validator accepted mixed release-profile pins:\n{combined}")
...
def main() -> int:
    ...
    negative_mixed_bootstrap_profile()
    ...
    print("Public repository tests passed (positive scan + 44 negative fixtures).")
    return 0
