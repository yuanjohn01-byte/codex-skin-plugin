param(
  [string]$HelperPath = "dist/helper/codex-skin-helper_0.1.0-paid-alpha.16_windows_x64.exe",
...
$target = Join-Path $scratch "codex-skin-helper_0.1.0-paid-alpha.16_windows_x64.exe"
$tampered = Join-Path $scratch "codex-skin-helper_0.1.0-paid-alpha.16_windows_x64.tampered.exe"
...
try {
  ...
  if (-not $beforeVersion.ok -or $beforeVersion.data.helperVersion -ne "0.1.0-paid-alpha.16") {
  ...
  if (-not $signedVersion.ok -or $signedVersion.data.helperVersion -ne "0.1.0-paid-alpha.16") {
  ...
  $summary = [ordered]@{
    ...
    artifact = [ordered]@{
      filename = "codex-skin-helper_0.1.0-paid-alpha.16_windows_x64.exe"
