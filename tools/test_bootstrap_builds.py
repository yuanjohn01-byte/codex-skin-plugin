#!/usr/bin/env python3
"""Reproducibility and format checks for Bootstrap cross-build artifacts."""
...
BUILDER = ROOT / "tools" / "build_bootstrap.py"
...
def main() -> int:
    with tempfile.TemporaryDirectory(prefix="codex-skin-bootstrap-builds-") as directory:
        root = Path(directory)
        ...
        if platforms != {"macos-arm64", "macos-x64", "windows-x64"}:
            raise AssertionError(f"unexpected Bootstrap target set: {platforms}")
        ...
        if any(
            item.get("helperReleaseTag") != "helper-v0.1.0-paid-alpha.10"
