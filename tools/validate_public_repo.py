...
from release_profiles import PROFILES
...
REQUIRED_SCRIPTS = {
    ...
    PLUGIN_ROOT / "scripts/bootstrap-pins.sh": (
        "Generated by tools/render_bootstrap_pins.py",
        "bootstrap_release_profile=",
        "bootstrap_api_host=",
        "bootstrap_sha256=",
    ),
    PLUGIN_ROOT / "scripts/bootstrap-pins.ps1": (
        "Generated by tools/render_bootstrap_pins.py",
        "$bootstrapReleaseProfile =",
        "$bootstrapAPIHost =",
        "$bootstrapSHA256 =",
    ),
}
...
EXPORTED_CONTRACTS = (
    ...
    (
        Path("contracts/helper-release-verification-keyset-v1.schema.json"),
        "codex-skin/contracts/public/helper-release-verification-keyset-v1.schema.json",
    ),
    (
        Path("contracts/helper-release-verification-keys-v1.json"),
        "codex-skin/contracts/public/helper-release-verification-keys-v1.json",
    ),
)
EXPORTED_FIXTURES = (
    (
...
EXPORTED_EMBEDS = (
    ...
    (
        Path("internal/release/trusted-verification-keys-v1.json"),
        "codex-skin/contracts/public/helper-release-verification-keys-v1.json",
    ),
)
...
def bootstrap_pin_value(content: str, assignment: str) -> str | None:
    match = re.search(assignment, content, flags=re.MULTILINE)
    return match.group(1) if match is not None else None


def validate_bootstrap_pin_profile(root: Path, errors: list[str]) -> None:
    shell_path = root / PLUGIN_ROOT / "scripts/bootstrap-pins.sh"
    powershell_path = root / PLUGIN_ROOT / "scripts/bootstrap-pins.ps1"
    ...
    try:
        ...
        return
    shell_values = {
        "profile": bootstrap_pin_value(shell, r"^bootstrap_release_profile='([^'\r\n]+)'$"),
        "host": bootstrap_pin_value(shell, r"^bootstrap_api_host='([^'\r\n]+)'$"),
        "tag": bootstrap_pin_value(shell, r"^bootstrap_release_tag='([^'\r\n]+)'$"),
        "version": bootstrap_pin_value(shell, r"^bootstrap_version='([^'\r\n]+)'$"),
    }
    powershell_values = {
        "profile": bootstrap_pin_value(powershell, r'^\$bootstrapReleaseProfile = "([^"\r\n]+)"$'),
        "host": bootstrap_pin_value(powershell, r'^\$bootstrapAPIHost = "([^"\r\n]+)"$'),
        "tag": bootstrap_pin_value(powershell, r'^\$bootstrapReleaseTag = "([^"\r\n]+)"$'),
        "version": bootstrap_pin_value(powershell, r'^\$bootstrapVersion = "([^"\r\n]+)"$'),
    }
    profile_name = shell_values["profile"]
    profile = PROFILES.get(profile_name or "")
    expected = (
        {
            "profile": profile.name,
            "host": profile.api_host,
            "tag": profile.helper_release_tag,
            "version": profile.bootstrap_version,
        }
        if profile is not None
        else None
    )
    if expected is None or shell_values != expected or powershell_values != expected:
        errors.append("Bootstrap pins do not match one fixed Staging or Production release profile")
...
def validate_scripts(root: Path, candidates: set[Path], errors: list[str]) -> None:
    for relative, markers in REQUIRED_SCRIPTS.items():
        ...
        network_tokens = ("curl ", "wget ", "Invoke-WebRequest", "http://", "https://")
        network_entry = relative in {
            PLUGIN_ROOT / "scripts/codex-skin.sh",
            PLUGIN_ROOT / "scripts/codex-skin.ps1",
        }
        if any(value in content for value in network_tokens):
            if not network_entry:
                errors.append(f"Paid Alpha wrapper must not implement network access: {relative}")
            elif (
                "https://github.com/yuanjohn01-byte/codex-skin-plugin/releases/download/"
                not in content
                or "http://" in content
                or (
                    "bootstrap_release_tag" not in content
                    and "$bootstrapReleaseTag" not in content
                )
                or "bootstrap" not in content.lower()
            ):
                errors.append(f"Paid Alpha Bootstrap wrapper network origin is not fixed: {relative}")
    validate_bootstrap_pin_profile(root, errors)
...
def validate_exported_contracts(root: Path, candidates: set[Path], errors: list[str]) -> None:
    ...
    helper_trusted_contract = Path("contracts/helper-release-verification-keys-v1.json")
    helper_trusted_embed = Path("internal/release/trusted-verification-keys-v1.json")
    if artifacts[helper_trusted_contract] != artifacts[helper_trusted_embed]:
        errors.append("embedded Helper release verification trust root differs from the public contract")
